...
*/
ob_start();
$token = '1126517370:AAHUwwooU3WFdyPhY9r6veMM4uW_MfToW2Y';
...
*/
  $status = bot('getChatMember',['chat_id'=>'@ESCADA12','user_id'=>$chat_id])->result->status;
  if($status == 'left'){
   bot('sendMessage',[
       'chat_id'=>$chat_id,
       'text'=>"عذرا عزيزي... يجب الإشتراك في القناة حتى تتمكن من إستخدام البوت...🙋‍♂
إشترك هنا chs12345678900987654321
ثم إضغط /start 👉       'reply_to_message_id'=>$message->message_id,
   ]);
   exit();
  }
